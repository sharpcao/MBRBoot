...
void CPalette::draw_rect(uint x, uint y,uint width, uint height, Color8 color, char* buf)
{
...
void CPalette::draw_cursor()
{
    draw_rect(_mouse_x,_mouse_y, 16,16,Color8::COL8_008484,_cursor_image);
}
...
void CPalette::move_cursor_by(int dx, int dy)
{
    _mouse_x = (uint) set_inrange(_mouse_x + dx, 0,_xsize);
    _mouse_y = (uint) set_inrange(_mouse_y + dy, 0,_ysize);
}

void CPalette::hide_cursor()
{
    boxfill(Color8::COL8_008484,_mouse_x,_mouse_y,_mouse_x+16,_mouse_y+16);
}