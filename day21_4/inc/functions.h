...
public:
	...
	bool is_empty() const {return _idx == 0; }
	...
	bool insert(uint at, char asc){
		if(_idx < _max-1 && at <= _idx ){
			_buf[++_idx] = 0;
			for(uint i = _idx-1; i > at; --i ) _buf[i] = _buf[i-1];
			_buf[at] = asc;
		}else{
			return false;
		}
...
public:
	...
	bool has(const T& v);
	bool is_full() { return (_last == _end); }
	void reset() { _last = 0;}
...
private:
	int _total = 0, _cur = 0;
	Array<stringbuf<>> _data;
public:
	stringbuf<> get_prev();
	stringbuf<> get_next();
	void add_cmd(const stringbuf<>& scmd);
